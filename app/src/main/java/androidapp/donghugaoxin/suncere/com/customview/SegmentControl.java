...
import suncere.androidapp.lib.customview.RadiusDrawable;
...
public class SegmentControl extends View {
    ...
    public SegmentControl(Context context, AttributeSet attrs, int defStyle){
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, suncere.androidapp.lib.R.styleable.segmentcontrol);

        String textArray = ta.getString(suncere.androidapp.lib.R.styleable.segmentcontrol_texts);
        ...
        mTextSize = ta.getDimensionPixelSize(suncere.androidapp.lib.R.styleable.segmentcontrol_android_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14, context.getResources().getDisplayMetrics()));
        mColors = ta.getColorStateList(suncere.androidapp.lib.R.styleable.segmentcontrol_colors);
        mCornerRadius = ta.getDimensionPixelSize(suncere.androidapp.lib.R.styleable.segmentcontrol_cornerRadius, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context.getResources().getDisplayMetrics()));
        mDirection = Direction.values()[ta.getInt(suncere.androidapp.lib.R.styleable.segmentcontrol_direction, 0)];

        backgroundColor=ta.getColor(suncere.androidapp.lib.R.styleable.segmentcontrol_backgroundColor, Color.TRANSPARENT);
        
        mHorizonGap = ta.getDimensionPixelSize(suncere.androidapp.lib.R.styleable.segmentcontrol_horizonGap, 0);
        mVerticalGap = ta.getDimensionPixelSize(suncere.androidapp.lib.R.styleable.segmentcontrol_verticalGap, 0);

        selectedTextColor=ta.getColor(suncere.androidapp.lib.R.styleable.segmentcontrol_selectTextColor,Color.WHITE);
        unselectedTextColor=ta.getColor(suncere.androidapp.lib.R.styleable.segmentcontrol_unselectTextColor,Color.BLACK);
        
        
        int gap = ta.getDimensionPixelSize(suncere.androidapp.lib.R.styleable.segmentcontrol_gaps, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, context.getResources().getDisplayMetrics()));
        ...
        mBackgroundDrawable = new RadiusDrawable(mCornerRadius, true, backgroundColor);
    ...
    public void onDraw(Canvas canvas){
        ...
        if(mTexts != null && mTexts.length > 0){
            for(int i = 0; i < mTexts.length; i++){
                ...
                { 
                	if (unselectedTextColor!=Color.BLACK) {//使用自定义颜色  否则采用跟mpaint上的设置同颜色  目的是防止之前的APP设置有所改变   该修改在做甘肃项目时进行修改
                		int preColor=mPaint.getColor();
